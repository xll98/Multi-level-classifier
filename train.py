...
from utils.utils import show_config,get_lr_scheduler,set_optimizer_lr,get_num_list
from nets.multi_level_class import MultiLevelClassifier
...
if __name__ == "__main__":
    ...
    # --------------------------------------------------------#
    annotation_path = "./labels/cls_train.txt"
    # --------------------------------------------------------#
    ...
    #   载入模型并加载预训练权重
    # ---------------------------------#
    model = MultiLevelClassifier(backbone=backbone, level1_num_classes=level1_classes,
                                 level2_num_classes_list=level2_classes_list,
                                 level3_num_classes_list=level3_classes_list)
    if model_path != '':
        if local_rank == 0:
